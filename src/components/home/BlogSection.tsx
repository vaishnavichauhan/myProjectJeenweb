  ...
  return (
    <section className="pt-12 pb-16 lg:pt-16 lg:pb-20 bg-white relative overflow-hidden border-b border-slate-200/80" id="insights">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <FadeIn direction="up" distance={20} className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12 pb-6 border-b border-slate-200/80">
          ...
          <Link
            ...
            className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-[#FAF8F5] hover:bg-[#1A3B71] text-[#1A3B71] hover:text-white font-bold text-xs uppercase tracking-wider transition-all border border-slate-200 shadow-2xs group flex-shrink-0 self-start md:self-auto hover:scale-[1.02] active:scale-95"
          >
        ...
        <StaggerContainer staggerDelay={0.1} className="grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8">
          {BLOG_POSTS.map((post) => (
            <StaggerItem key={post.id}>
              <article
                className="bg-[#FAF8F5] rounded-3xl p-0 border border-slate-200/90 shadow-sm hover:shadow-xl hover:-translate-y-1.5 transition-all duration-500 flex flex-col justify-between group relative overflow-hidden h-full"
              >
                <div>
